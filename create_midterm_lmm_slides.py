...
def sl_crustal_vector(prs, page):
    ...
    s = slide_base(prs, "Ⅰ. LMM 중간결과",
                   "항공자력의 방향 정보 — 쓸 수 있으나 아직 진단용이다",
                   "총자력 이상은 이상벡터를 주자기장 방향으로 투영한 값이라 "
                   "역산할 수 있다. 다만 격자 결측을 0 으로 채운 역산이라 생산 "
                   "모형에는 넣지 않는다.", page, rule=BLUE)
    ...
    y = BODY_Y + 0.66
    rows = [
        ["LOO 편각 D", "0.5123°", "0.5022°", "−0.010°"],
        ["LOO 복각 I", "0.1972°", "0.0881°", "−0.109°"],
        ["가설 A 상관", "—",
    ...
    band(s, M, y + 0.24, 7.55, 1.34,
         "성능만 보면 넣는 쪽이 낫다 — 특히 복각이 0.197 → 0.088° 로 줄어든다.\n"
         "   그럼에도 생산 모형에서 뺀 이유는 신뢰성이다. 격자 결측 39% 를 0 nT 로 "
         "채운 역산이라\n   복원 진폭에 불확실성이 있다. 결측 보간을 개선한 뒤 "
         "다시 판정한다.")
    return s
...
def sl_compliance(prs, page):
    ...
    rows = [
        ...
        ["Regional 적합", "다항식 또는 R-SCHA",
         "상수항(0차) — 공간항·시간항 모두 미채택", "미달"],
        ["지각 결합", "항공자력 이상을 F 에 반영",
         "1:1 결합(α=1). α 추정은 과적합으로 기각", "준수"],
