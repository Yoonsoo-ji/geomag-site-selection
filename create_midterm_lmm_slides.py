...
def sl_residual(prs, page):
    ...
    band(s, M, y + 0.28, 7.55, 1.20,
         "가설 A 는 처음에 「규모 미달」로 기각됐었다.\n"
         "   방위 기준이 틀린 방문 2건이 잔차를 부풀려 상대적 비중을 가렸기 때문이다.\n"
         "   그 둘을 걷어내자 예측이 잔차의 절반을 차지하게 됐다 — 순서가 중요했다.")
...
def sl_crustal_vector(prs, page):
    ...
    s = slide_base(prs, "Ⅰ. LMM 중간결과",
                   ...
                   "역산하면 편각·복각에 쓸 수 있다.", page, rule=GREEN)
    ...
    y = BODY_Y + 0.66
    rows = [
        ["LOO 편각 D", "0.5403°", "0.3396°", "−37%"],
        ["LOO 복각 I", "0.2600°", "0.2408°", "−7%"],
        ["편각 잔차 RMS", "20.73′", f"{ASY['rD_rms']:.2f}′", "−34%"],
        ["가설 A 상관", "r=+0.560 p=0.046",
         f"r={HYP['A_vector_D']['r']:+.3f} p={HYP['A_vector_D']['p']:.3f}", "강화"],
        ["적용 후 잔여 상관", "—",
    ...
    stat(s, 10.63, BODY_Y + 0.66, 2.10, 1.24, "1.60", "최적배율 — 진폭이 모자란다",
         PEACH, ORANGE, 21)
    band(s, 8.30, BODY_Y + 2.05, 4.43, 1.62,
         "배율 1.6 이 다음 요구사항이다\n\n"
         "예측 방향은 맞는데 크기가 모자란다.\n"
         "2.8 km 격자가 단파장을 평활했기 때문 —\n"
         "250 m 원측선 자료가 곧 다음 이득이다.", "23456B")
    band(s, M, y + 0.24, 7.55, 1.34,
         "⚠ 복각 LOO 가 0.043° 로 표시되는 경우가 있으나 그대로 읽으면 안 된다.\n"
         "   잔차가 줄면서 강건 적합의 채택 측점이 15 → 8 로 좁아졌고, 교차검증은 그 "
         "8점 위에서만 계산된다.\n"
         "   채택 집합을 고정해 견준 값이 위 표의 −7% 다. 편각은 같은 방식으로 "
         "−37% 라 선택효과가 아니다.")
    return s
...
def sl_roadmap(prs, page):
    ...
    steps = [
        ...
        ("3", "원측선 · 신규 측점", "KPI 미달분 · 독립 표본 검증", "진행", BLUE),
    ]
    bw = (CW - 3 * 0.16) / 4
...
def sl_risk(prs, page):
    ...
    prio = [
        ("최우선", "항공자력 원측선(250 m)",
         "지각 벡터 진폭이 1.6배 모자라다 — 편각·총자력 공통 병목", ORANGE),
        ("2순위", "잔차 큰 측점 재측정",
         f"편각 잔차 {ASY['rD_rms']:.1f}′ — 방위표지 고정과 함께", BLUE),
        ("3순위", "신규 측점 확보 (30점)",
...
def sl_compliance(prs, page):
    ...
    band(s, M, y + 0.20, CW, 1.12,
         ...
         "· 단일 관측소로는 외부장을 못 뗀다 → 4소 공간보간으로 편각 45% 개선, "
         "다만 교란시엔 여전히 깨진다", GREEN)
    return s


def sl_external_verdict(prs, page):
    """관측소 4소 공간보간으로 편각에 적용."""
    s = slide_base(prs, "Ⅲ. 향후 검증·보완",
                   "청양 한 곳으로는 안 됐고, 네 곳을 쓰니 됐다",
                   "자문 권고대로 대체관측소를 모형에 실제로 넣었다. 다만 정온 세션에 "
                   "한정할 때만 이득이 난다.", page, rule=GREEN)
    rows = [
        ["미적용", "—", "전 세션", "0.5899°", "기준선"],
        ["편각만 차감", "청양 단독", "전 세션", "0.3653°", "종전 채택"],
        ["편각만 차감", "관측소 4소", "전 세션", "0.4028°", "단독보다 나쁨"],
        ["편각만 차감", "청양 단독", "정온만", "0.3763°", ""],
        ["편각만 차감 (채택)", "관측소 4소", "정온만", "0.3232°", "최종 채택"],
    ]
    y = table(s, M, BODY_Y, CW,
              ["방식", "보정량 자료원", "세션", "LOO 편각 D", "판정"], rows,
              [0.24, 0.18, 0.13, 0.20, 0.25], row_h=0.46, head_fill=GREEN)
    stat(s, M, y + 0.20, 3.90, 1.06, "−45%",
         "미적용 대비 편각 교차검증 오차", LBLUE, GREEN, 22)
    stat(s, M + 4.05, y + 0.20, 3.90, 1.06, "178세션",
         "4소 보간으로 산출한 보정량", LGRAY, BLUE, 20)
    band(s, M + 8.10, y + 0.20, CW - 8.10, 1.06,
         "왜 정온만인가\n\n교란시엔 4소 평면적합이 발산한다\n(총자력 편차 최대 567 nT)",
         "23456B")
    band(s, M, y + 1.34, CW, 0.82,
         "계기 좌표계 정합이 관건 — 강릉·제주 Y 는 변화계 기준(Y≈0)이라 회전을 "
         "맞춰야 한다(제주 −6.32° · 강릉 −8.10° · 이천 −0.69° · 청양 −0.03°).\n"
         "⚠ 다섯 설정 중 교차검증 최소를 고른 것이므로 0.365 → 0.323 차이 자체가 "
         "아니라 방식의 타당성이 근거다.", GREEN)
    return s
...
def sl_advisory(prs, page):
    ...
    s = slide_base(prs, "Ⅲ. 향후 검증·보완",
                   ...
                   rule=GREEN)
    rows = [
        ["일변화 미보정이 근본 원인", "야장 직접 대조로 원시값 확정 + 편각 보정 적용",
         "반영"],
        ["청양 단독 차감의 한계", "관측소 4소 공간보간으로 교체 (앞 장)", "반영"],
    ...
    band(s, M, y + 0.18, CW, 0.90,
         "남은 두 가지는 자료·장비 사안이다.\n\n"
         "· 항공자력 원측선(250 m) — 지각층 진폭 부족의 유일한 해법\n"
         "· 신규 측점 확보 — 배치 최적과 자기환경 등급이 어긋나는 구간은 후보 재탐색이 "
         "필요하다", GREEN)
    return s
...
def sl_resolution(prs, page):
    ...
    s = slide_base(prs, "Ⅱ. 확인·추가 자료",
                   "항공자력 제원 확인 — 원측선 1~2 km, 공개본 2.5 km 격자",
                   "박영수 외(2019) 「한국의 자력이상도」에 37년치 탐사 제원이 정리돼 "
                   "있다. 원측선을 받아도 개선폭은 제한적이다.", page, rule=RED)
    ...
    band(s, M, y + 0.18, CW, 1.66,
         ...
         "조밀한 자료가 아니었다.\n\n"
         "① 원측선을 받아도 측선 직교방향은 2.8 → 1~2 km, 1.4~2.8배 개선에 그친다 "
         "— F 50 nT 달성은 여전히 불확실\n"
         "② 편각 D 0.1° 는 이 한계와 무관하다 — 지각층은 F 에만 기여한다", RED)
    return s
