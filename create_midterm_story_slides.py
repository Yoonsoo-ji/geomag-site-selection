...
def s07_selection(prs, page):
    s = slide_base(prs, "Ⅲ. 검증",
                   ...
                   "모두 같은 조건(평가 측점 고정)에서 처음부터 다시 적합해 "
                   "비교했다.", page, rule=GREEN)
    y = BODY_Y
    rows = [
        ["Regional 차수", "R0 상수항", "0.4987°", "채택"],
        ["", "R1 1차 평면", "0.5236°", "기각"],
        ["", "R2 2차", "0.6389°", "기각"],
        ["", "R1T 시간항 추가", "0.5459°", "기각"],
    ]
    y = table(s, M, y, 5.95, ["구분", "후보", "LOO 편각", "판정"], rows,
              [0.26, 0.34, 0.22, 0.18], row_h=0.40, size=10)
    rows2 = [
        ["지각 결합", "F0 미사용", "72.3 nT", "기각"],
        ["", "F1 1:1 결합", "63.0 nT", "채택"],
        ["", "Fα 계수 추정(α=0.67)", "67.0 nT", "기각"],
    ]
    table(s, 6.75, BODY_Y, CW + M - 6.75,
          ["구분", "후보", "LOO 총자력", "판정"], rows2,
          [0.24, 0.36, 0.22, 0.18], row_h=0.40, size=10)
    rows3 = [
        ["미적용", "—", "0.5123°", "기준선"],
        ["E0", "청양 단독", "0.5495°", "기각"],
    ...
    y2 = max(y, BODY_Y + 1.70) + 0.24
    text(s, M, y2, CW, 0.30, "External — 단일 관측소로는 안 되고, 공간보간이라야 된다",
         12.5, NAVY, True)
    y2 = table(s, M, y2 + 0.34, 5.95, ["모델", "자료원", "LOO 편각", "판정"],
               rows3, [0.20, 0.40, 0.22, 0.18], row_h=0.40, size=10)
    band(s, 6.75, y2 - 1.94, CW + M - 6.75, 1.90,
         "선택의 요점\n\n"
         "· 측점 16개로는 «공간 구조»를 지지할 수 없다 → 상수항\n"
         "· 지각 결합계수를 자유롭게 두면 과적합 → 1:1\n"
         "· 「가장 가까운 관측소와 같다」는 가정은 성립하지 않는다 → 공간보간\n\n"
         "복잡한 모형이 훈련 오차만 낮추고 예측을 악화시키는 사례를 세 번 확인했다",
         "23456B")
    return s
...
def s11_vector(prs, page):
    ...
    rows = [
        ["벡터 미적용 (현재)", "0.4987°", "기준"],
        ["벡터 적용 · zero 채움", "0.4506°", "−0.048°"],
        ["벡터 적용 · mean 채움", "0.4507°", "−0.048°"],
        ["벡터 적용 · harmonic 채움", "0.4739°", "−0.025°"],
        ["결측 경계 측점 배제", "0.4638°", "−0.035°"],
        ["블록 홀드아웃 (남·중·북)", "0.4848 → 0.4560°", "−0.029°"],
    ]
    y = table(s, M, y, 6.60, ["조건", "LOO 편각", "변화"], rows,
              [0.46, 0.30, 0.24], row_h=0.40, size=10)
    ...
    band(s, M, y + 0.24, 6.60, 0.90,
         "이 슬라이드의 요지는 「미완」이 아니라 「개선 경로가 이미 확인됐다」는 것이다.")
    return s
