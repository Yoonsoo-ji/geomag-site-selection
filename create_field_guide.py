...
def content():
    ...
    A(("callout",
       ...
       "빠집니다."))
    A(("p", "GSM-19T 는 판독할 때마다 시각을 기기에 저장하지만, 판독 화면에는 시각이 "
            "나오지 않습니다. 그래서 카드의 시각은 기록하시는 분이 센서에서 떨어져 "
            "시계를 보고 적습니다(시계는 자성 물품이라 센서 곁에 두면 안 됩니다). "
            "사무실에서는 기기 파일의 시각을 정본으로 쓰고, 카드 시각으로 어느 줄이 "
            "어느 판독인지 맞춥니다."))
    A(("callout",
       "GSM-19T 는 꺼 둔 지 24시간이 지나면 날짜와 시각이 지워집니다. 매일 켜자마자 "
       "GNSS 시각에 맞춰 KST 로 넣어 주시고, 카드 1 구역에 몇 초 차이였는지 적어 "
       "주세요. 이걸 빼먹으면 그날 기기 파일의 시각이 전부 틀어집니다."))
    ...
    A(("p", f"수평은 한 방향에 {len(SP.H_OFFSETS_M)}개 지점({offs} m)을 재고, 방향이 "
            ...
            f"번씩 재는 것이 기본입니다 — 국제 지침(IAGA)의 예시도 측점마다 판독 "
            f"하나입니다. 다만 신호품질 뒷자리가 0 이면 쓸 수 없는 판독이니 다시 "
            f"재 주세요."))

    A(("h1", "2. 이것만은 꼭 — 빠지면 되살릴 수 없는 것"))
    A(("p", "아래 항목들은 나중에 사무실에서 어떤 방법으로도 복원할 수 없습니다. "
            "실제로 예전 야장 68건을 열어 보니 총자력 결과 절 36건에 날짜는 적혀 "
            "있었지만 시각 칸은 모두 00:00 이었고, 그 때문에 그날 «언제» 쟀는지를 몰라 "
            "시간에 따른 보정을 제대로 할 수 없었습니다."))
    ...
    A(("h2", "1 구역 — GSM-19T 설정 확인"))
    A(("table", ["항목", "이렇게 둡니다", "왜"],
       [["측정 모드", "Mobile", "점마다 버튼을 눌러 재는 모드입니다. Base 는 자력계가 "
                                 "두 대일 때 한 대를 세워 두는 모드입니다."],
        ["기기 시각", "매일 KST 로 맞춤", "꺼 둔 지 24시간이 지나면 지워집니다."],
        ["GPS 옵션", "끔(없으면 「없음」)", "켜 두면 시각이 UTC 로 바뀌고 좌표도 GPS 값이 "
                                        "되어 0.5 m 간격을 가를 수 없습니다."],
        ["좌표 방식", "X/Y · P0 = (0, 0)", "동·북은 +, 서·남은 − 입니다."],
        ["AC 필터", "60 Hz", "전원선 간섭을 줄입니다(우리나라는 60 Hz)."],
        ["튜닝", "수동 «50»", "자동 탐색은 구배가 큰 곳에서 실패할 수 있습니다."],
        ["센서 방향선", "수직", "우리나라는 복각이 약 53도라 세워서 씁니다."]],
       [16, 22, 62]))
    ...
    A(("steps", [
        "센서 높이를 정해 막대를 조립하고, 그 높이를 카드에 적습니다. 모든 측점에서 "
        "같은 높이로 잽니다.",
        "중심점(P0) 좌표를 (0, 0)으로 두고 한 번 재어, 시각·값·품질을 적습니다.",
        f"동쪽으로 {SP.H_OFFSETS_M[0]} m, {SP.H_OFFSETS_M[1]} m, 그다음부터는 "
        f"1 m 씩 {SP.H_OFFSETS_M[-1]} m 까지 옮겨 가며 잽니다.",
        "다시 중심점으로 돌아와 좌표를 (0, 0)으로 다시 넣고 한 번 더 잽니다. "
        "「1」(반복) 키는 좌표를 그대로 두기 때문에 그냥 누르면 10 m 좌표로 저장됩니다.",
    ...
    A(("p", "중심점을 앞뒤로 두 번 재는 이유는, 그 사이에 자기장이 저절로 변한 만큼을 "
            ...
            "값인 것처럼 계산됩니다."))
    A(("p", "한 방향을 마치면 P0 앞뒤 두 값을 직접 빼 보시면 좋습니다. IAGA 지침은 "
            "중심점 값이 «몇 nT» 넘게 바뀌면 다시 재거나 Variometer·관측소 자료로 "
            "보정하라고 합니다. 이번에는 그 기준을 파일럿에서 정하니, 얼마나 "
            "벌어졌는지 소견에 적고 여유가 되면 차수를 올려 다시 재 주세요. 다시 "
            "재실 때도 앞서 적은 값은 지우지 마시고 그대로 두시면 됩니다."))
    A(("note", "「품질」 칸에는 판독 화면 오른쪽 위의 두 자리 숫자를 적습니다(99 가 "
               "가장 좋음). 뒷자리가 0 이면 쓸 수 없는 판독이라 「1」(반복) 키로 다시 "
               "재고, 다시 잰 값은 비고에 적어 주세요."))
    ...
    A(("h1", "5. 철수하기 전에"))
    A(("p", "카드 3 구역 아래에 「완전성 점검」 줄이 있습니다. 일곱 가지를 스스로 "
    ...
    A(("table", ["점검 항목", "확인할 것"],
       [["필수행 다 채움", "빈 칸으로 남은 측점이 없는지"],
        ...
        ["기기 시각 맞춤", "오늘 기기 시각을 KST 로 맞추고 차이를 적었는지"],
        ["품질 뒷자리 0 없음", "쓸 수 없는 판독을 다시 재지 않고 남겨 두지 않았는지"],
