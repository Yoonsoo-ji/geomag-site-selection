...
function lmm(lat, lon, elev, year){
  ...
  // 지각층은 결합계수 α 를 곱해 더한다(모형 정의). α 는 모델 파일에서 읽는다.
  const alpha = (LM.crustal && LM.crustal.alpha !== undefined)
                ? LM.crustal.alpha : 1;
  const crF = (cr.value === null ? 0 : alpha * cr.value);
  const D = D0 + crD + dD, I = I0 + crI + dI, F = F0 + crF + dF;
  ...
  return {
    ...
    layers:{core:F0, crustal:crF, crustalRaw:cr.value, crustalAlpha:alpha,
            crustalStatus:cr.status, regional:dF,
...
function run(){
  ...
  document.getElementById('lay').innerHTML = [
    ...
    ['③ Crustal (KIGAM 항공자력'
     + (L.crustalAlpha!==1 ? `, α=${L.crustalAlpha.toFixed(2)}` : '') + ')',
...
document.getElementById('acc').innerHTML =
  `<b>정확도 고지</b> — 중간보고 기준 버전(<b>Midterm Baseline v1</b>)입니다.<br>
   <span style="font-size:12px;opacity:.85">구성 — Regional 상수항(R0) ·
   ...
   <ul style="margin:6px 0 0 18px;padding:0">
     <li><b>기존 반복측량의 편각 계통오차</b> — 재방문마다 마크 참방위각이 옮겨 가는
         ...
         제외했으나, 남은 잔차의 우세 원인으로 본다</li>
     <li><b>지각 벡터 미반영</b> — 스칼라 이상에서 벡터를 복원하면 편각이 일관되게
         좋아지나(0.499°→0.45~0.47°), 격자 결측 39 % 를 0 으로 채운 역산이라
         현재는 <b>진단 전용</b>이다. 원자료 결측 해결이 승격 조건</li>
     <li><b>Crustal 층 해상도</b> — 보유 격자는 1.5분(약 2.8 km) 전국 컴필레이션.
         지표 점 잔차와의 상관 r=${SUM.crust_r.toFixed(2)} 이나 표본이
         ${SUM.crust_n}개뿐이다. 측선간격 250 m 원측선 자료는 존재하지 않아
         현 격자가 해상도 상한</li>
     <li><b>지표 측점 부족</b> — 유효 ${SUM.n_sites}개 측점 (권고 30점).
         이 때문에 지역장의 <b>공간 구조를 자료로 결정할 수 없어</b> 상수항을 쓴다</li>
   </ul>
