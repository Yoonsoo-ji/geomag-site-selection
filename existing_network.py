...
# 기존 30점 중 **선점 대상 16점**.
# 나머지 14점은 저수지(제방)에 설치된 점으로 판정되어 선점에서 뺀다 —
# 지반 거동·수위 변동으로 표석 지속성이 확보되지 않는다.
#
# ⚠️ **함양은 2026-09-02 에 「기타」에서 옮겨 왔다**(사용자 지시). 종전 15점
# 판정은 저수지 설치 여부만 보고 일괄로 뺀 것이고, 함양은 선점 검토 대상으로
# 다시 넣는다. 이 변경으로 선점 검토 지점이 **A 6 + B 도엽대표 28 + 기존점 16
# = 50** 이 된다.
EXISTING_TARGET = [
