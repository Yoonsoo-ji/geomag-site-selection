...
━━━ 모델 구축 우선순위 가중치 ━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  - 지형 대표성, 전자기 이격도, 자기이상·지질경계 모델 기여도
...
# 자기이상·지질경계 모델 기여도 (KIGAM 1.5분 격자, 반경 0.05°≈5.5km 내 P90-P10)
# ■ 평가 스케일: 반경 0.05°(≈5.5 km) — KIGAM 1.5' 최소 실용 반경
# ■ 지표: P90-P10 (이상치 강건 범위, robust range)
# ■ 평가 목적: 단순 균등 배치에 따른 관측망 한계 보완 — 후보지 주변 자기이상
#   변동폭을 활용한 로컬 지자기모델 구축 기여도 평가
...
#   400~800 nT   → 7점  (고그레디언트 — 자연기원+인공잡음 혼재, 현장 검증 필수)
...
def compute_anomaly_variation_zones(
    ...
    ■ 평가 기준 (자기이상·지질경계 모델 기여도 평가):
      ...
      후보지 주변 자기이상 변동폭을 활용한 로컬 지자기모델 구축 기여도 평가
      한반도 실측 분포: 중앙값 126 nT, 평균 164 nT, P99 ≈ 625 nT
      > 800 nT : 조건부 제외 또는 현장 정밀조사 대상 (전체의 약 1% 극단값)
      그 외 구간은 compute_priority()에서 모델 기여도 기반 점수화
      (<30:5점 / 30~150:8점 / 150~400:10점 / 400~800:7점)
    ...
    (function(){
      ...
      window.addEventListener('load',function(){
        ...
        function syncTitle(){
          ...
          if(t) t.style.visibility=(window.innerWidth<=768 &&
            c.classList.contains('leaflet-control-layers-expanded'))?'hidden':'';
        }
        document.addEventListener('click',function(e){
          if(!c.contains(e.target)) c.classList.remove('leaflet-control-layers-expanded');
          setTimeout(syncTitle,30);
        }, true);
