...
<script>
(function(){
  try{if(window.innerWidth<=760){var t0=document.getElementById('titleBar');if(t0)t0.classList.add('collapsed');}}catch(e){}
  function build(){
    ...
    if(t){var tc=t.querySelector('.t-chev');
      function tpaint(){tc.innerHTML=t.classList.contains('collapsed')?'▸ 제목':'▾';}
      t.addEventListener('click',function(){t.classList.toggle('collapsed');tpaint();});
      tpaint();}
  }
...
def add_existing_layer(m, merged=None):
    """기존 지자기 측정점(EXISTING_USE 30) 토글 — 두 레이어로 분리.
      ...
      · ⭐ 기타 기존점 (14) — 저수지(제방) 설치점. 종전 ⭐ 마커, 기본 끔.
