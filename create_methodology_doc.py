...
def main():
    ...
    run = info.add_run(datetime.now().strftime("%Y년 %m월") + "\n\n")
    run.font.size = Pt(11)
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["", "② 지형적 대표성", "15", "✅",
             "Open-Elevation API 경사도 (1 - slope/30°) × 15"],
            ["환경 정온도", "③ 전력/철도 이격도", "15", "✅",
             "log(dist) 함수 기반 정규화"],
            ["", "④ 인구 밀집 이격도", "15", "✅",
             "log(dist) 함수 기반 정규화"],
            ...
            ["", "⑥ 암상 적합성", "5", "✅",
             "수치지질도(1:250,000) 자성암종·단층 이격거리 log 정규화"],
            ["운영 인프라", "⑦ 부지 지속성", "10", "❌",
             "국공유지 여부·개발 제한도 (미확보)"],
            ["", "⑧ 관리 접근성", "5", "❌",
             "도로망·통신망 거리 (미확보)"],
    ...
    doc.add_heading("② 지형적 대표성 (15점)", level=3)
    doc.add_paragraph(
        "완만한 지형일수록 측정 장비 설치와 안정적인 지면 접촉이 용이하며 "
        "지형 기인 자기 왜곡도 줄어든다. Open-Elevation API로 후보점 주변의 "
        "수치 표고를 취득하여 경사도를 계산한다."
    )
    add_styled_table(doc,
        ["단계", "처리 내용"],
        [
            ["1", "후보점 좌표(WGS84)로 Open-Elevation API 호출 (반경 ~5 km, 청크 단위)"],
            ["2", "인접 격자 고도값으로 경사도(°) 계산"],
            ["3", "점수 = (1 - slope / 30°) × 15, 0~15점 클리핑"],
            ["4", "API 취득 실패 지점은 중간값 7.5점으로 대체"],
        ],
        col_widths=[2, 15],
    )
    doc.add_paragraph(
        "참고: 경사도 0° → 15점(최고), 경사도 30° 이상 → 0점. "
        "결과는 data/dem_slopes.json에 캐시되어 재실행 시 재사용된다."
    )
    ...
    doc.add_heading("⑤ 자기이상·지질경계 모델 기여도 (10점, 조건부)", level=3)
    doc.add_paragraph(
        "단순 균등 배치에 따른 관측망 한계를 보완하기 위하여 후보지 주변 반경 0.05°(≈5.5 km) 내 "
        "자기이상 변동폭(P90-P10)을 활용하여 로컬 지자기모델 구축 기여도를 평가한다. "
        "변화가 매우 작은 지역은 관측 안정성은 높으나 모델 보정 기여도가 제한적이므로 중간 점수를 부여하고, "
        "150~400 nT 수준의 중간 그레디언트 지역은 지질경계, 해안부, 자기장 변화 구간을 반영할 가능성이 높아 "
        "최고점을 부여한다. 다만 400 nT 이상 고그레디언트 지역은 자연기원 자기이상과 인공잡음 가능성이 "
        "혼재하므로 현장 전자력 측정 및 주변 자기장 탐사를 통해 최종 적합성을 확인한다."
    )
    add_styled_table(doc,
        ["단계", "처리 내용"],
        [
            ["1", "후보점 좌표를 WGS84로 변환"],
            ["2", "KIGAM 격자에서 반경 0.05° 내 자기이상값 추출 (≥3개 격자 필요)"],
            ["3", "해당 영역의 P90-P10 변동폭(robust range) 계산"],
            ["4", "구간별 점수 부여 (모델 기여도 기준)"],
        ],
        col_widths=[2, 15],
    )
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["30 ~ 150 nT", "8점", "저~중 그레디언트 — 안정 측정 + 일정한 기여도"],
            ["150 ~ 400 nT", "10점", "중간 그레디언트 — 지질경계·해안부·변화 구간 반영 (최고점)"],
            ["400 ~ 800 nT", "7점", "고그레디언트 — 자연/인공 혼재 가능, 현장 검증 필수"],
            ["> 800 nT", "조건부 제외", "극단값 — 사전 제외 또는 현장 정밀조사 대상"],
            ["데이터 미취득", "6점", "중립값 (KIGAM 격자 부족 지점)"],
        ],
        col_widths=[4, 3, 10],
    )
    ...
    run.font.name = "맑은 고딕"
    run = p.add_run(
        "KIGAM 1.5분(≈2.8 km) 광역 자력이상도 기반 예비선정이며, "
        "400 nT 이상 고그레디언트 지역 및 >800 nT 조건부 제외 지역은 "
        "현장 전자력 측정 및 주변 자기장 탐사를 통해 최종 적합성을 확정한다."
    )
    run.font.size = Pt(9)
    run.font.color.rgb = RGBColor(0x66, 0x66, 0x66)
    run.font.name = "맑은 고딕"

    doc.add_heading("⑥ 암상 적합성 (5점)", level=3)
    doc.add_paragraph(
        "KIGAM 수치지질도(1:250,000) 기반으로 자성 암종 및 단층으로부터의 "
        "이격 거리를 점수화한다. 자성 암종(현무암·반려암·화산암류 등) 폴리곤 내부 및 "
        "단층 버퍼 500m 이내 지점은 제외 구역 필터에서 이미 제거되며, "
        "잔여 후보점에 대해 경계까지의 거리를 log 정규화하여 점수화한다."
    )
    add_styled_table(doc,
        ["단계", "처리 내용"],
        [
            ["1", "각 후보점에서 자성 암종 경계까지 최소 거리 d_rock (UTM, m) 계산"],
            ["2", "각 후보점에서 단층 버퍼 경계까지 최소 거리 d_fault (UTM, m) 계산"],
            ["3", "log(1 + d) 변환 후 최대값 정규화"],
            ["4", "점수 = (d_rock 정규화 × 0.5 + d_fault 정규화 × 0.5) × 5"],
            ["", "※ 한쪽 데이터만 가용 시 해당 거리만으로 5점 산정"],
        ],
        col_widths=[2, 15],
    )
    doc.add_paragraph(
        "참고: 멀리 떨어질수록 자성 암종·단층 기인 국소 자기이상 영향이 줄어 "
        "안정적인 총자력 측정 환경이 확보된다."
    )
    ...
    add_styled_table(doc,
        ["구분", "포함 항목", "가용 만점", "비고"],
        [
            ["전체 가용", "①(25)+②(15)+③(15)+④(15)+⑤(10)+⑥(5)", "85점", "85점 → 100점 정규화"],
            ["KIGAM 미배치", "①(25)+②(15)+③(15)+④(15)+⑥(5)", "75점", "75점 → 100점 정규화"],
            ["DEM 취득 실패", "①(25)+③(15)+④(15)+⑤(10)+⑥(5)", "70점", "70점 → 100점 정규화"],
            ["최소 구성", "①(25)+③(15)+④(15)", "55점", "55점 → 100점 정규화"],
        ],
        col_widths=[3.5, 7, 2.5, 4],
    )
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["자기이상도", "KIGAM 1.5분 격자\nmag_1982-2018_1.5min_ed.dat", "DAT",
             "data/mag_1982-2018_1.5min_ed.dat"],
            ["수치표고(DEM)", "Open-Elevation API\n(SRTM 기반)", "JSON",
             "data/dem_slopes.json (캐시)"],
            ["수치지질도", "KIGAM 1:250,000 전국\n(수동 배치)", "SHP",
             "data/수치지질도_25만축척_전국/"],
        ],
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["s2_지형", "② 지형 경사도 점수", "/ 15"],
            ["s5_모델기여도", "⑤ 자기이상·지질경계 모델 기여도 점수 (KIGAM)", "/ 10"],
            ["s6_암상", "⑥ 암상 적합성 점수 (수치지질도)", "/ 5"],
            ["dem_slope_deg", "후보점 경사도", "°"],
            ["d_power_km", "전력 제외 구역까지 이격 거리", "km"],
            ["d_railway_km", "철도 제외 구역까지 이격 거리", "km"],
            ["d_urban_km", "도시·주거 제외 구역까지 이격 거리", "km"],
            ["d_geo_fault_km", "단층 버퍼 경계까지 이격 거리", "km"],
        ],
        col_widths=[3.5, 9, 3],
    )
    ...
    doc.add_paragraph(
        "현재 미산정 항목(총 15점)은 해당 데이터 확보 시 코드 내 "
