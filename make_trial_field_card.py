...
`make_trial_survey_book.py` 가 내는 17시트 야장은 **자료 관리 스키마**다 —
...
| 절대측정 세션 D·I·F·시각 | §13② 이격 증빙 4행/점 |
| 중심점 최종 위치·사유 | 기존점 성분별 대조가능성 |
| 현장 소견 | 검토 반영 추적표 |
...
def build_card(wb, p, idx):
    ...
    # ── 6. 절대측정 ─────────────────────────────────────────
    r = _s(ws, r, "6. 절대측정 세션 (1일 6회 이상 · 회당 20분 이내)",
           "⚠ 편각·복각을 잴 때 총자기장과 «시각»을 함께 잰다. F 는 세션마다 적고, "
           "DI점과 5 m 이상 떨어진 원격점에서도 한 번씩 재어 시각과 함께 적는다.")
    _lbl(ws, f"A{r}", "회차");      _lbl(ws, f"B{r}", "시작 시각")
    _lbl(ws, f"C{r}", "종료 시각"); _lbl(ws, f"D{r}", "소요(분)")
    _lbl(ws, f"E{r}", "편각 D(gon)"); _lbl(ws, f"F{r}", "복각 I(gon)")
    _lbl(ws, f"G{r}", "F(nT)");     _lbl(ws, f"H{r}", "F 측정 시각")
    _lbl(ws, f"I{r}", "F 위치")
    r += 1
    # ⚠️ 회차 라벨을 「1회」로 적는다 — 그냥 「1」이면 3절의 거리 1 m 행과 이름이
    #    겹쳐 사람도 스크립트도 헷갈린다(실제로 시험에서 걸렸다).
    for k in range(1, 9):
        c = ws[f"A{r}"]
        c.value, c.font, c.fill, c.border, c.alignment = (
            f"{k}회", F_LBL, FILL_LBL, BOX, AL_C)
        _fld(ws, f"B{r}", "hh:mm:ss")
        _fld(ws, f"C{r}", "hh:mm:ss")
        _fld(ws, f"D{r}", "0.0",
             calc=f'=IF(COUNT(B{r}:C{r})<2,"",(C{r}-B{r})*1440)')
        for cc in "EFG":
            _fld(ws, f"{cc}{r}", "0.0000" if cc in "EF" else "0.0")
        _fld(ws, f"H{r}", "hh:mm:ss")
        _fld(ws, f"I{r}")
        _dv(ws, f"I{r}", ["DI점", "원격점"])
        r += 1
    ws.merge_cells(f"A{r}:I{r}")
    c = ws[f"A{r}"]
    c.value = "⚠ 「소요(분)」이 20 을 넘으면 그 회차는 규정 밖이다 — 다시 잰다."
    c.font, c.fill, c.alignment = F_WARN, FILL_WARN, AL_TL
    r += 2
