...
⚠️ **판독은 위치당 1회다** — 종이에 손으로 쓰는 것이 원본이므로 64위치 × 3회를
받아 적는 것은 현실적이지 않다. 기기가 내부에서 평균하고 IAGA 예시도 점당
단일값이다. 값이 의심스러우면 **차수를 올려 그 방향을 통째로 다시 잰다.**
"""
...
def build_card(wb, p, idx):
    ...
    r = _s(ws, r, "0. 측정 전 확인 — 자기오염 방지",
           ...
           "시계·펜·혁대 하나가 네 방향을 같은 쪽으로 밀어 공간구배로 «오인»되게 "
           "만든다. (「지구물리측량 작업규정」 §18③ 을 참고한 실무 지침이며, "
           "이 탐사는 법정 선점이 아니다)")
    ...
    r = _s(ws, r, "1. 도착 · 관측 조건 · 원시자료 연결",
           "⚠ **야간 관측은 하지 않는다**(발주자 결정). Kp 와 예보는 «기록만» 하고 "
           "배제 기준으로 쓰지 않는다 — 어느 값에서 걸러야 하는지 아직 정해지지 "
           "않았고, 그 기준을 정하려고 이 탐사를 하는 것이다. 교란이 뚜렷하면 "
           "그 구간을 빼거나 다시 재고 소견에 적는다.")
    ...
    ws.merge_cells(f"A{r}:I{r}")
    c = ws[f"A{r}"]
    c.value = ("⚠ 이 카드가 «원본»이다. 장비 원시파일은 나중에 자동 반입하므로 "
               "파일명과 레코드 범위를 반드시 적는다 — 종이와 기기 원본을 잇는 "
               "유일한 끈이다.")
    c.font, c.fill, c.alignment = F_WARN, FILL_WARN, AL_TL
    ws.row_dimensions[r].height = 14
    r += 2
    ...
    r = _s(ws, r, "3. 수평 자기구배 — 방향마다 P0 재측정",
           ...
           f"참고값: 반경 10 m 내 «4개 방위 측선의 관측점에서» "
           f"{SP.IAGA_RANGE_NT:.0f} nT · 구배 {SP.EURO_GRAD_NT_PER_M:.0f} nT/m "
           f"— 국제 권고일 뿐이고 **국내 기준은 없다**. 이 탐사가 그 기준을 만들 "
           f"자료를 모으는 것이므로, 참고값을 넘었다고 현장에서 탈락시키지 말고 "
           f"«그대로 기록»한다. 측선 사이와 대각선은 재지 않으니 「반경 10 m "
           f"전체」라고 말할 수도 없다. 판정은 사무실에서 한다.")
