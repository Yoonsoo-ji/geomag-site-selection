...
def s_axes(slide):
    ...
    for i, (tt, col, lines) in enumerate([
        ("항공자력 포함(4축)의 효과", GREEN,
         ...
         "· 평가 측점 고정 교차검증에서 편각 0.512° → 0.502°, 복각 0.250° → 0.220°\n"
         "· 개선폭은 작으나 두 성분에서 같은 방향"),
        ("3축 수준에 머무르는 제약", RED,
...
def s_service(slide):
    ...
    note(slide, y + 0.12,
         "※ 도폭 내 편각 변화는 IGRF-14 로 산출 / 도폭 규격은 지도도식적용규정 "
         "제4조·제211조(1:50,000 = 15′) / 도엽 대표값은 도곽 중심 좌표 기준으로 "
         "산출하고 도폭 내 변화폭을 함께 제공")
