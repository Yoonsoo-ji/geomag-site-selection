...
def sheet_gate(wb, pts):
    ...
    note(ws, 2, "N", "⚠ **이 탐사는 법정 선점(§14 선점실시)이 아니고 판정 기준도 없다**"
                     "(발주자 결정 2026-09-07). 아래 조항 이름은 «참고한 실무 지침»일 "
                     "뿐이며 어떤 칸도 «법정 요건 통과»를 뜻하지 않는다. 이 시트가 "
                     "실제로 묻는 것은 하나다 — **나중에 분석할 수 있을 만큼 기록이 "
                     "갖춰졌는가.** 정량 지표는 ⑬에서 본다.", warn=True)
