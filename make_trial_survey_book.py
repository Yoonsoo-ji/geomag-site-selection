...
def sheet_gate(wb, pts):
    ...
    title(ws, "N", "이번 단계 적용요소 · 기록 완전성 게이트 — 통과/실패로만")
    note(ws, 2, "N", "⚠ 정량 임계로 바로 선점을 확정하지 않기 위해 판정을 두 층으로 나눈다. "
                     "이 시트는 «반드시 통과해야 하는 것»만 다룬다 — 법정 조문과 기록 "
                     "완전성이다. 정량 지표는 ⑬에서 본다.", warn=True)
    note(ws, 3, "N", "⚠ **「법정 요건 통과」로 읽지 말 것.** §17 은 0.1 nT 판독 외에 "
                     "지자기 3요소·방위각·시간·온습도 측정능력을, §21 은 동일시간 "
                     "상시기준점 비교와 기준년 환산까지 규정한다. 이 단계는 발주자 "
                     "결정에 따라 그 «일부만» 적용한다. 정식 표지 설치라면 §15·§16, "
                     "정식 측정 준비라면 §18 의 적용범위를 따로 판단해야 한다"
                     "(⑮ 확인 필요).", warn=True)
