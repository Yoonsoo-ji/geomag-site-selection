...
# ⚠️ 이름이 값과 어긋나면 안 된다(코덱스 M9) — 앞의 두 칸은 «구배»가 아니다.
#    P0 후 − 전   = 같은 자리를 처음·끝에 재서 생긴 차이(시간변화 점검)
#    F 최대−최소  = 그 방향에서 나온 값의 폭(범위)
HEAD = ["방향", "잰 거리", "P0 후 − 전\n(nT)", "잰 구간 F\n최대−최소\n(nT)",
        "최대 구간\n수평구배\n(nT/m)", "발생구간", "P0→끝\nΔF (nT)",
        "P0→끝\n평균구배\n(nT/m)", "비고"]
TAIL = ("「P0 후 − 전」은 중심점을 처음과 끝에 두 번 재서 생긴 차이입니다 — 자리가 달라져서가 "
        "아니라 재는 동안 자기장이 흔들린 정도이니, 이 값이 크면 다른 칸에도 그만큼 섞여 "
        "있습니다. 「F 최대−최소」도 구배가 아니라 그 방향에서 나온 값의 폭입니다. 거리로 "
        "나눈 것은 구간 구배뿐입니다. 「잰 거리」는 마지막으로 값을 적은 거리이고, 「P0→끝」 "
        "두 칸은 그 거리까지의 차이와 평균입니다.")
