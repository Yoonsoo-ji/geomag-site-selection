...
def main():
    ...
    lines = [
        ...
        ("      총자력 F 에 1:1 로 결합합니다. 결합계수를 자료로 추정해 보았으나", None),
        ("      (α = 0.67) 교차검증 오차가 커져 α = 1 을 유지했습니다.", None),
        ("      스칼라 이상에서 벡터를 복원해 편각·복각에 반영하는 방안은 격자", None),
        ("      결측 39% 를 0 으로 채운 역산이라 현재 진단 전용으로 둡니다.", None),
        ...
        ("      1차 평면으로 공간보간해 측점 위치의 편차를 추정하고, 편각은 편각", None),
        ("      관측 구간, 복각은 복각 관측 구간의 보정량으로 각각 뺐습니다.", None),
        ("      단일 관측소·최근접 관측소는 오히려 기준선보다 나빠 채택하지 않았고,", None),
        ("      상시관측소 축 규약 문서 확보 전까지 잠정 적용입니다.", None),
        ("      총자력에는 적용하지 않습니다 — 야장에 F 관측시각이 없습니다.", None),
        ("", None),
        ("2. 표고 보정", f_lbl),
