...
# ── 기존점에서 수행한 사전 답사 — 기존점으로 합친다 ──────────────────────────
#
# 사전 답사는 신규 후보지뿐 아니라 **기존 지자기점에서도** 실시했다. 그 카드는
# 후보지가 아니라 「그 기존점을 현장에서 확인한 기록」이므로, 등급 마커로 따로
# 세우면 같은 표석이 두 번 계상된다 — DS-047 「화천 기존점」과 관측망 화천은
# 실제로 **12 m** 떨어진 같은 자리였다.
#
# → 등급 판정(`review`)은 그대로 두되 **선점 후보 집계·지도 등급 레이어에서
#   빼고**, 답사 내용은 기존점 팝업·엑셀에 붙여 남긴다.
...
# ── 등급 수동 승격 ───────────────────────────────────────────────────────────
#
# ⚠️ `JUDGMENT_OVERRIDE`(종합판정 자체를 바꿈)와 다르다. 카드의 종합판정은
# 그대로 두고 **선점 검토 등급만** 올린다. 근거를 반드시 적을 것.
#
#   DS-098 문산 신규점 — 카드 종합판정은 「조건부 적합」(자연환경 1건)이나
#     자기교란 8항목이 모두 '없음'이고 방위표지 가능·최장 94 m 로 B 문턱
#     (100 m)에 근접한다. 문산 도엽에 다른 후보가 없어 **그 도엽의 유일한
#     선점 후보**이기도 하다. (2026-09-02 사용자 지시)
GRADE_OVERRIDE = {
    "DS-098": ("B", "문산 도엽 유일 후보 · 자기교란 없음 · 방위표지 최장 94 m"),
}
...
def review(d):
    ...
    kd = key_disturb(d)
    op = d["판정의견"].replace("\n", " ").strip()
    if d.get("관리번호") in GRADE_OVERRIDE:
        g, why = GRADE_OVERRIDE[d["관리번호"]]
        md = mark_max_dist(d)
        dist = f" (현 최장 {md:.0f}m<{MARK_MIN_DIST:.0f}m)" if md is not None else ""
        concl = {"A": "선점 가능 — 자기구배 조사 진행",
                 "B": f"조건부 선점 가능 — 방위표지 거리 확보 필요{dist}"}.get(
                     g, f"등급 {g}")
        return g, concl, f"※등급 승격: {why}" + (f" · {op}" if op else "")
    if v == "조사 미완료":
