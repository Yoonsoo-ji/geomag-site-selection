...
def main():
    ...
    add_callout(
        ...
        "측선간격 250 m 원측선 자료를 사용하였다. KPI 달성에는 원측선 자료가 필요하다.")
    ...
    add_callout(
        ...
        "따라서 관측 시각을 알고 외부장을 보정하지 않으면 편각 KPI 달성은 원리적으로 불가능하다. "
        "성과표에는 관측연도만 기록되어 있어 2022~2025 야장 확보가 선행되어야 한다.")
    ...
    add_styled_table(
        doc,
        ["순위", "필요 조치", "기대 효과"],
        [["1", "2022~2025년 야장 확보 → 관측 일시 복원",
          "④ External 층 적용 가능. 편각 KPI 달성의 전제조건"],
         ["2", "KIGAM 항공자력 원측선 자료 (측선간격 250 m)",
          "③ Crustal 층 교체. 현재 2.8 km 격자로는 단파장 신호 표현 불가"],
         ["3", "CYG 자료의 NOC 모드 분해 (최소 1년, 권장 2022~현재)",
          "외부장의 공간투영. 단순 차감의 한계 극복"],
         ["4", "2027 신규 캠페인 30점 측정",
          "권고 측점 수 충족. DI 플럭스게이트 방위기준 관리 강화 필요"]],
        center_from=1)
