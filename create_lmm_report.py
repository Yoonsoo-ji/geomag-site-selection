...
def main():
    ...
    add_styled_table(
        ...
        [["① Core", "IGRF-14 (13차 구면조화)", "≳3,000 km", "확보"],
         ...
         ["④ External", "관측소 4소 1분 자료 (청양·제주·강릉·이천)",
          "일·시·분", "편각에 적용"]],
        center_from=2)
    para(doc, "③ Crustal 층은 총자력에 1:1 로 결합한다. 결합계수를 자료로 추정해 "
              "보았으나(α = 0.67) 교차검증 오차가 오히려 커져 α = 1 을 유지하였다. "
              "스칼라 이상에서 벡터 3성분을 복원해 편각·복각에 반영하는 방안은 "
              "격자 결측을 0 으로 채운 역산이라 현재는 진단 전용으로 둔다. "
              "④ External 층은 관측 성과 정리 단계에서 편각·복각에 적용한다 — 모델 자체는 "
    ...
    para(doc, "현재 방침은 관측소 4소(청양·제주·강릉·이천)의 정온야간 기준선 대비 "
              ...
              "편각은 편각 관측 구간, 복각은 복각 관측 구간의 보정량으로 각각 빼는 "
              "것이다. 총자력에는 적용하지 않는다 — 야장에 F 관측시각이 없어 세션 "
              "전체 구간으로 대신할 수밖에 없기 때문이다. 층이 결합되어 있어 순차 "
              "튜닝이 성립하지 않으므로 방식마다 처음부터 다시 적합하고, 평가 측점을 "
              "한 번만 정해 고정한 채 교차검증으로 비교하였다.")
    add_styled_table(
        doc,
        ["방식", "보정량 자료원", "LOO 편각 D", "판정"],
        [["미적용", "—", "0.5123°", "기준선"],
         ["편각만", "청양 단독 (E0)", "0.5495°", "기각"],
         ["편각만", "최근접 관측소 (E1)", "0.5504°", "기각"],
         ["편각·복각", "관측소 4소 (E2)", "0.4987°", "채택(잠정)"],
         ["편각·복각", "4소 역거리가중", "0.5595°", "기각"]])
    para(doc, "단일 관측소(E0)와 최근접 관측소(E1)는 오히려 기준선보다 나쁘다. "
              "「가장 가까운 상시관측소와 변화가 같다」는 가정이 한반도 규모에서 "
              "성립하지 않는다는 뜻이며, 공간보간만이 이득을 낸다. 다만 개선폭이 "
              "0.014°로 작고 여러 설정 중 최소값을 고른 것이므로, 채택 근거는 폭이 "
              "아니라 방식의 물리적 타당성에 있다. 상시관측소의 좌표축·기준선 규약 "
              "문서를 확보하기 전까지 잠정(provisional) 채택으로 표기한다.", size=9.5)
    ...
    doc.add_heading("7.6 일변화 보정 적용 여부 — 확인 완료", level=2)
    para(doc,
         "지구물리측량 작업규정 제21조는 「동일시간에 상시 측정 기준점의 측정값과 "
         "비교 검토」하여 일변화와 영년변화를 보정하도록 정한다. 성과표에 이미 보정이 "
         "적용되어 있다면 본 연구가 다시 보정할 때 이중보정이 되므로, 이 여부를 두 가지 "
         "방법으로 직접 확인하였다.")
    add_styled_table(
        doc,
        ["확인 방법", "내용", "결과"],
        [["① 야장 직접 대조",
          "성과표 값과 야장 세션평균을 매칭 18행에서 비교",
          "편각 차 RMS 0.00′ · 복각 0.03′ · 총자력 0.02 nT"],
         ["② 상시관측 회귀",
          "같은 날 여러 세션의 관측값 변동을 상시관측 편차로 회귀(170세션)",
          "기울기 −0.02 [−0.22, +0.18]"]])
    add_callout(
        doc, "확정 — 성과표에는 일변화 보정이 적용되어 있지 않다",
        "① 이 결정적이다. 성과표 값이 야장 세션평균과 표기 자릿수 이내로 일치하므로, "
        "그 사이에 어떤 보정도 들어가지 않았다. 일변화 보정이 적용되었다면 그 크기만큼"
        "(편각 약 6′, 총자력 약 38 nT) 차이가 났어야 한다. 따라서 본 연구가 외부장을 "
        "빼는 것은 이중보정이 아니다. 동일 측점 재방문 총자력 산포가 최대 138 nT 에 "
        "이르는 것도 이 결론과 부합한다.")
    para(doc,
         "② 는 독립적인 확인으로 설계했으나 판정 근거로는 쓰지 않는다. 야장 값은 "
         "정의상 원시 판독값이므로 이 회귀의 기울기는 「보정 여부」가 아니라 「우리 "
         "외부장 추정이 실제 세션 간 변동을 얼마나 맞히는가」를 재기 때문이다. "
         "기울기가 0 에 가깝다는 것은 보정이 되어 있다는 뜻이 아니라, 추정한 보정량이 "
         "실제 변동과 어긋난다는 뜻이다. 이 결과는 아래 7.6.1 에서 따로 다룬다.",
         size=9.5)

    doc.add_heading("7.6.1 외부장 추정의 한계 — 크기는 맞고 시점이 어긋난다", level=2)
    para(doc,
         "같은 측점·같은 날의 세션들은 측점 계통오차와 그날의 기준값을 정확히 "
         "공유하므로, 세션 간 차이는 그 시각의 외부장과 측정 잡음만 남는다. "
         "이 조건에서 관측된 편각의 세션 간 산포는 2.61′ 이고, 우리가 추정한 외부장 "
         "편차의 산포는 1.97′ 로 크기가 비슷하다. 그러나 둘의 상관은 −0.02 로 사실상 "
         "없다.")
    add_callout(
        doc, "외부장 보정을 잠정 적용으로 두는 이유",
        "추정한 보정량의 규모는 타당하나 시각별 변동을 맞히지 못한다. 관측소 4개소를 "
        "1차 평면으로 보간하는 현재 방식이 측점 위치의 실제 변동을 재현하지 못한다는 "
        "뜻이며, 설명자료가 요구하는 공간투영(NOC)이 필요한 이유이기도 하다. "
        "교차검증에서 관측된 개선(편각 0.512°→0.499°)이 작은 것도 이와 일관된다. "
        "따라서 ④ External 층은 잠정(provisional) 적용으로 표기한다.")

    doc.add_heading("7.6.2 확인이 필요한 사항", level=2)
    bullets(doc, [
        "상시관측소 좌표축·기준선 규약 문서 — 관측소마다 성분 정의가 달라(청양·이천은 "
        "지리축, 강릉은 계기축, 제주는 벡터가 닫히지 않음) 현재는 IGRF 로 회전각을 "
        "추정하고 있다. 공식 문서로 대체해야 한다.",
        "총자력 측정 시각과 센서 위치차 — 2020 절차는 편각·복각 측정점에서 5 m 이상 "
        "떨어져 측정한 뒤 센서 위치에서 재측정하여 위치차를 보정하도록 한다. 야장 68개를 "
        "전수 조사한 결과 F 측정시각은 전부 미기입이고 두 지점의 값도 남아 있지 않다. "
        "향후 야장 양식에 해당 칸을 추가해야 한다.",
