...
def build(d):
    ...
    booktabs(doc,
             ...
             [["① Core 층 구현",
              ...
              ["지각층의 각도 기여",
               f"진단 전용 · 예상 기여 {vr['predD_rms_arcmin']:.1f}′",
               "KPI 6′ 초과 — 결측 보완 후 재평가"],
              ["편각 잔차 원인",
    ...
    booktabs(doc,
             ["양", "값"],
             [["현행 모형의 편각 잔차 RMS (벡터 미적용)", f"{hA['rms']:.2f}′"],
             ...
             [8.0, 5.0], aligns=["l", "r"])
    note(doc,
         "표 읽는 법: 생산 모형은 지각 벡터를 D·I 에 넣지 않는다(설계안 v2 12항 — "
         "격자 결측 39 % 를 0 nT 로 채운 역산이라 진단 전용으로 둔다). 따라서 "
         f"「차감 전」이 현행 모형의 편각 잔차({asym['rD_rms']:.2f}′)이고, "
         "「차감 후」는 벡터를 넣었을 때의 가상값이다.")
    rich(doc, [
    ...
    booktabs(doc,
             ["단계", "내용", "비고"],
             [["0", "측점·장비·관측 이력 감사 (station version · 등급)",
               "완료 — data/station_history.csv"],
              ["0-2", "2022~25 야장 확보 및 관측시각 복원", "완료"],
